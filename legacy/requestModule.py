...
class Manager():
    ...
    def setMenu(self):
        ...
        for word in soup.find_all("div", class_="daily-menu"):
            ...
            if count < 6:
                self.weekends[count].data[u"학식"][u"점심"] = menu
            elif count < 12:
                self.weekends[count % 6].data[u"학식"][u"저녁"] = menu
            elif count < 18:
                self.weekends[count % 6].data[u"기식"][u"점심"] = menu
            elif count < 24:
                self.weekends[count % 6].data[u"기식"][u"저녁"] = menu
            elif count < 30:
                self.weekends[count % 6].data[u"교직원(생활관)"][u"점심"] = menu
            elif count < 36:
                self.weekends[count % 6].data[u"교직원(생활관)"][u"저녁"] = menu
            elif count < 42:
                self.weekends[count % 6].data[u"교직원(종합관)"][u"아침"] = menu
            elif count < 48:
                self.weekends[count % 6].data[u"교직원(종합관)"][u"점심"] = menu
            elif count < 54:
                self.weekends[count % 6].data[u"교직원(종합관)"][u"저녁"] = menu
            count += 1
