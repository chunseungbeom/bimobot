...
def _getSoup():
    r = requests.get("http://www.ajou.ac.kr/kr/life/food.jsp")
...
def _soupToMenus(soup):
    ...
    '''
    cols = [[[]] * 10] * 6
    menus = [_tagTostr(i).split() for i in soup.find_all("ul", class_="tri_list02")]
    for item in menus:
...
def _soupToSubtitles(soup):
    ...
    '''
    subtitles = [_tagTostr(i) for i in soup.find_all("th", class_="no_right yesback04")]
    return subtitles
